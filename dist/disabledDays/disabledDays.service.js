"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DisabledDayService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const disabledDay_schema_1 = require("./schema/disabledDay.schema");
const mongoose_2 = require("mongoose");
let DisabledDayService = class DisabledDayService {
    disabledDayModel;
    constructor(disabledDayModel) {
        this.disabledDayModel = disabledDayModel;
    }
    async create(createDisabledDayDto) {
        try {
            const createdDisabledDay = new this.disabledDayModel(createDisabledDayDto);
            return await createdDisabledDay.save();
        }
        catch (error) {
            throw new Error(`Error disabling day: ${error.message}`);
        }
    }
    async findAll() {
        try {
            return await this.disabledDayModel.find().exec();
        }
        catch (error) {
            throw new Error(`Error fetching disableDays: ${error.message}`);
        }
    }
    remove(date) {
        try {
            return this.disabledDayModel.findOneAndDelete({ date }).exec();
        }
        catch (error) {
            throw new Error(`Error removing disabled day ${date}: ${error.message}`);
        }
    }
};
exports.DisabledDayService = DisabledDayService;
exports.DisabledDayService = DisabledDayService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(disabledDay_schema_1.DisabledDay.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], DisabledDayService);
//# sourceMappingURL=disabledDays.service.js.map